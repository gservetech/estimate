...
const ProductCard = ({ product, own }: { product: Product; own: boolean }) => {
  ...
  return (
    <>
      <Head>
        <title>{product?.name} - GserveTech</title>
        <meta property="og:description" content={Array.isArray(product?.description) ? product.description.join(", ") : product?.description || "High-quality product available on GserveTech"} />

        <meta property="og:url" content={`https://www.gservetech.com/product/${product.id}`} />
        <meta property="og:type" content="product" />
        <meta property="og:title" content={product?.name} />
       
        <meta property="og:image" content={product?.images[0]} />
        <meta property="og:image:alt" content="Product Image" />
        <meta property="og:site_name" content="GserveTech" />
      </Head>
        ...
        <div className="p-4 flex flex-col gap-2">
              ...
              {Array.from({ length: 5 }).map((_, index) => {
                ...
                return (
                  <LuStar
                    ...
                    key={index}
                    className={`${isLastStar ? "text-gray-500" : "text-lightOrange"} w-3 h-3`}
                  />
                );
              })}
            </div>
          ...
          <div className="text-center">
            ...
            <p className="text-xs text-gray-600">
              {product?.stock ?? 0 > 0 ? `Ships in ${product.shippingTime || 2} days` : "Out of Stock"}
            </p>
          </div>
          <div className="flex flex-col">
            ...
            <Button
              ...
              className={twMerge("bg-darkBlue/10 border text-black border-green-500 w-full py-2 mt-2 rounded-md font-medium hover:bg-green-500 hover:text-white")}
            >
              View Product
            </Button>
          </div>
